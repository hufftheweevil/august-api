...
module.exports = function setup(config = {}) {
  let { AUGUST_API_KEY, AUGUST_PN_SUB_KEY, AUGUST_INSTALL_ID, AUGUST_PASSWORD, AUGUST_ID, COUNTRY_CODE } =
    process.env
  ...
  if (countryCode = 'US') {
    ...
    let apiKey = config.apiKey ?? AUGUST_API_KEY ?? DEFAULT_API_KEY_NON_US
  }
  if (countryCode = 'US') {
    let pnSubKey = config.pnSubKey ?? AUGUST_PN_SUB_KEY ?? DEFAULT_PN_SUB_KEY
  } else {
    let pnSubKey = config.pnSubKey ?? AUGUST_PN_SUB_KEY ?? DEFAULT_PN_SUB_KEY_NON_US
  }
